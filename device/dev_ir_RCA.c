...
static int InfraRed_RX_RCA_Calculate(IR_TypeDef *pIR_Obj)
{
  ...
  uint16_t min0, max0, min1, max1;
	...
	min0 = IR_RCA_Zero[1].us * 0.8;
	max0 = IR_RCA_Zero[1].us * 1.2;
	min1 = IR_RCA_One[1].us * 0.8;
	max1 = IR_RCA_One[1].us * 1.2;
	
	if(pIR_Obj->rx_len != 0)
	{
		for(idx=sizeof(IR_RCA_Head)/sizeof(IR_RCA_Head[0])+1; idx<pIR_Obj->rx_len; idx+=2)
		{			
