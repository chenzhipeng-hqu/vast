...
/**
  ...
  * @retval 
  */
int16_t EasyKey_Init(EasyKey_HandleTypeDef *pKey)
{
...
uint8_t KeyProcess_Start (EasyKey_HandleTypeDef *pKey, EASYKEY_SEL_FUNCx selFunc)
{
	...
	switch(selFunc)
	{
		...
		{
			postToLog("%s click once, t:%d, %d\r\n", __FUNCTION__, HAL_GetTick(), pKey->Key_Time);
			break;
		}
		case EASYKEY_FUNC_CLICK_DOUB :
		{
			postToLog("%s click double, t:%d, %d, %d, %d, %d, %d, %d\r\n"	,
																	__FUNCTION__			,
																	HAL_GetTick()	,
																	pKey->Key_Time		,
		...
		{
			postToLog("%s press long, t:%d, %d\r\n", __FUNCTION__, HAL_GetTick(), pKey->Key_Time);
			break;
		}
		case EASYKEY_FUNC_CONTINOUS :
		{
			postToLog("%s continous, t:%d, %d\r\n", __FUNCTION__, HAL_GetTick(), pKey->Key_Time);
			break;
		}
		default:
		{
			postToLog("%s %d, err key func!\r\n", __FUNCTION__, __LINE__);
			break;
		}
	}
