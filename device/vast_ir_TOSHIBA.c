...
//
const IR_BufTypeDef IR_TOSHIBA_Head[] = {
	{4500, IR_PIN_HIGH},
	{4500, IR_PIN_LOW},
};

const IR_BufTypeDef IR_TOSHIBA_One[] = {
	{560,	 IR_PIN_HIGH},
	{1685, IR_PIN_LOW},
};

const IR_BufTypeDef IR_TOSHIBA_Zero[] = {
	{560, IR_PIN_HIGH},
	{560, IR_PIN_LOW},
};

const IR_BufTypeDef IR_TOSHIBA_Repeat[] = {
	{9000, IR_PIN_HIGH},
	{2250, IR_PIN_LOW},
	{560, IR_PIN_HIGH},
};
...
//
int InfraRed_RX_TOSHIBA_Calculate(IR_TypeDef *pIR_Obj)
{
	uint8_t idx = 0, byte = 0, _bit = 0x01;
	uint16_t min0, max0, min1, max1;
	uint8_t val[4] = {0};
	
	min0 = IR_TOSHIBA_Zero[1].timer - IR_TOSHIBA_Zero[1].timer/5;
	max0 = IR_TOSHIBA_Zero[1].timer + IR_TOSHIBA_Zero[1].timer/5;
	min1 = IR_TOSHIBA_One[1].timer - IR_TOSHIBA_One[1].timer/5;
	max1 = IR_TOSHIBA_One[1].timer + IR_TOSHIBA_One[1].timer/5;
	
	if(pIR_Obj->len != 0)
	{
		for(idx=sizeof(IR_TOSHIBA_Head)/sizeof(IR_TOSHIBA_Head[0])+1; idx<pIR_Obj->len; idx+=2)
		{			
